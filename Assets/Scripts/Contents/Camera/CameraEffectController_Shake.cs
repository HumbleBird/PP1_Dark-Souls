...
public partial class CameraEffectController : MonoBehaviour
{
    ...
    {
        m_ShakeTr.localPosition = m_vOrgPos;
        m_bCameraShake = false;
    ...
    void CameraLimit(bool _bOrgPosY = false)
    {
        ...
        if (camera.x - m_CameraFOV_X < LEFT)
            camera.x = LEFT + m_CameraFOV_X;
        else if (camera.x + m_CameraFOV_X > RIGHT)
            camera.x = RIGHT - m_CameraFOV_X;
